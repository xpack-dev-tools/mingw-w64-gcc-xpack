...
xpm run docker-build-development --config ${props.platform} -C ~/Work/xpack-dev-tools/mingw-w64-gcc-xpack.git/build-assets
`} </CodeBlock>
...
- <code>xpack-mingw-w64-gcc-15.2.0-2-{props.platform}.{props.platform === 'win32-x64' ? 'zip' : 'tar.gz'}</code>
- <code>xpack-mingw-w64-gcc-15.2.0-2-{props.platform}.{props.platform === 'win32-x64' ? 'zip' : 'tar.gz'}.sha</code>
...
xpm run deep-clean --config ${props.platform}  -C ~/Work/xpack-dev-tools/mingw-w64-gcc-xpack.git/build-assets && \\
xpm run docker-prepare --config ${props.platform} -C ~/Work/xpack-dev-tools/mingw-w64-gcc-xpack.git/build-assets && \\
xpm run docker-build --config ${props.platform} -C ~/Work/xpack-dev-tools/mingw-w64-gcc-xpack.git/build-assets
`} </CodeBlock>
